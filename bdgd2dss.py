...
def feeders_feasibility(feeders_list, limiar, pref):
    pref = pref.split('.')[0] # Retira a extensão do arquivo 
    ...
    def montar_planilha():
        ...
        for feeder in feeders_list:
            ...
            if energyBDGD == 0: perc=0
            else: perc = abs(((energymeteryear / energyBDGD) - 1 ) * 100)
            ...
            data_list.append({
                ...
                "Percent Difference": f"{perc:.2f}" if isinstance(perc, (int, float)) else "N/A"
            })
        ...
        for row in range(2, sheet.max_row + 1):  # Começar da linha 2 para evitar o cabeçalho
            cell = sheet[f"H{row}"]  # Coluna F contém 'Percent Difference'
